...
/**
 * Performs below task : 1. Reads dimension of the images which don't have and
 * discard smaller images 2. Sorts images based on the dimension
 * 
...
public class SizeFilter implements Filter {
	final static private int MINIMUM_LENGTH = 50; // minimum width AND height of
													// image
	...
	public HtmlDoc fetchImageSize(HtmlDoc doc) {
		...
		for (HtmlImage image : doc.getImages()) {
			...
			if (width == 0 || height == 0) {
				try {
					if (Util.validateURL(image.getUrl())) { // only if url is
															// valid
